...
Auditoría de integridad de la suite completa y arreglo de lo encontrado. La suite estaba en verde, pero **verde no era íntegro**: un test ejecutaba un `git push --force origin master` real, otros pisaban el estado del usuario, y varios no podían fallar o verificaban un código distinto del que decían. (2.14.29 es la rama de V7-12, `feat/v2.5.0-voice-roi-extractors--claude-v7-12`, todavía sin mergear.)

**Método**: análisis estático del AST de los 455 tests (asserts que no pueden fallar, `except` amplios, parches sobre atributos inexistentes, estado global sin restaurar, cuerpos duplicados) más un plugin de pytest que registró, test por test, llamadas a código de `yunta/`, subprocesos y su cwd, intentos de red, escrituras fuera de temporales y fugas de entorno/cwd/globals. La suite se corrió en un clon **sin remoto**, en orden normal e inverso. Donde había duda, se rompió el código a propósito para ver si el test lo detectaba.

### Peligrosos (efectos reales)
- **`tests/test_tools.py::test_blocklist_git_push_force_con_env` ejecutaba `git push --force origin master` de verdad**, desde la raíz del repo, en cada corrida de la suite y en el hook de cada commit. El comentario suponía que "fallará por no ser repo"; el cwd sí era el repo. Hoy era inofensivo solo porque `master` local coincidía con `origin/master`: bastaba mergear un PR en GitHub y correr la suite antes de actualizar `master` local para rebobinar GitHub. El plugin interceptó la llamada (único test que intentó pushear). En el reflog de `origin/master` hay exactamente **un** push no fast-forward en 50 entradas —09-09 22:15, que borró de GitHub el merge del PR #1 (`d807b34`)— un día después de que se agregó este test; la relación es circunstancial, no probada. El contenido del PR sobrevivió por otro camino. **Arreglo**: el test verifica que el comando llega a `subprocess.run` (no está bloqueado) con `subprocess.run` reemplazado; no ejecuta nada.
- **Sin configuración de pytest, `pytest` en la raíz recolectaba `scratch/`** (no versionado), incluido desde `yunta check --tests` y el hook. `scratch/test_live_transcribe.py` ejecuta `_load_dotenv()` al importarse: cargaba el `.env` real (credenciales, modelo, endpoint) en el proceso de tests. Además 3 tests no versionados de `scratch/` inflaban los conteos reportados (los "440" / "448" de v2.14.28 y del handoff incluyen esos 3). **Arreglo**: `testpaths = ["tests"]` en `pyproject.toml`.
- **26 tests escribían en el cwd**: `.yunta/session_state.json` (el estado de `yunta --resume`), `.yunta/scratch/` y archivos temporales de escritura atómica en la raíz y en `docs/`. Corriendo en el repo, cada suite pisaba la sesión real del usuario: el `session_state.json` del repo principal contenía una sesión falsa de test ("Subtarea: Actualizar función helper"). Seis tests ejecutaban además un `rm temporal.txt` real en la raíz. **Arreglo**: `tests/conftest.py` pone cada test en su propio directorio temporal. Solo 2 tests dependían del cwd (leían el `README.md` real para verificar que contuviera "yunta"); ahora crean el suyo.

### No probaban lo que decían
- **`test_missing_model_fails_clearly` verificaba otro código en cualquier worktree o rama.** Lanza `python -c "from yunta.provider ..."` desde un directorio temporal, donde `yunta` resuelve al paquete instalado en modo editable, que apunta al repo principal. **Demostrado por mutación**: con la regla 2 de `AGENTS.md` rota en el código bajo prueba (el provider elegía `openai/gpt-4o` en silencio al faltar el modelo), el test **seguía pasando**. La causa fue el fixture de aislamiento que agregó v2.14.25 (`chdir` a un temporal), del mismo autor que esta auditoría. **Arreglo**: `conftest.py` pone `PYTHONPATH` apuntando al checkout para toda la sesión —lo que corrige a la vez a todos los tests que lanzan `python` como subproceso— y el test ahora verifica de dónde importó el provider antes de construirlo, así que no puede volver a pasar en silencio contra otro código. `pythonpath = ["."]` en `pyproject.toml` hace lo mismo para el proceso de pytest.
- **`test_sentence_chunker_handles_floating_points_and_abbreviations` exigía el bug.** El nombre prometía que el troceador maneja bien abreviaturas, pero el assert pedía que `"El Dr."` quedara como oración aparte ("partición indeseada", decía su propio comentario): arreglar el bug hacía fallar el test. Tampoco verificaba el `2.7.5` que prometía el nombre. **Arreglo**: dos tests —uno que verifica que los números de versión no parten la oración, y uno con el comportamiento correcto para abreviaturas marcado `xfail(strict=True)` como bug conocido: si se arregla el troceador, el `xfail` pasa a fallar y avisa que hay que quitarlo.
- **`test_session_permissions_pattern_is_per_first_token` decía lo contrario de lo que verifica**, en un test de seguridad. El comportamiento cambió (ver "Fix: 'siempre' ahora aprueba la tool completa" más abajo): ya no se acota el permiso al primer token. Se invirtió el cuerpo pero quedó el nombre viejo. **Arreglo**: renombrado a `test_session_permissions_siempre_approves_the_whole_tool`. Ejecutaba además un `curl http://x` real (7.6s esperando DNS); ahora usa `echo`, que conserva la intención (un comando con otro primer token) sin red.
- **`test_extract_url_graceful` no podía fallar**: las tres salidas de la rama YouTube contienen "YouTube". Y corría `yt-dlp` real contra YouTube, que si fallaban los subtítulos intentaba bajar el audio del video y transcribirlo. **Arreglo**: `test_extract_url_routes_youtube_links_to_the_youtube_extractor`, sin red, verifica el enrutamiento contra el encabezado exacto del extractor de YouTube.

### Riesgos latentes (hoy no cambiaban ningún resultado)
- 12 tests dejaban variables de entorno cambiadas (`LLM_MODEL`, `OPENAI_API_KEY`, `LLM_API_BASE`, `LLM_MODELS`, `YUNTA_SESSION_ID`, `LLM_REASONING_EFFORT`), algunas escritas por el propio código de producción (la auto-reparación de V7-11 escribe `LLM_REASONING_EFFORT=off`; tres de esos tests son de v2.14.27, del mismo autor). 17 tests dejaban `litellm.completion`/`litellm.api_base` reemplazados (`setup_function` sin teardown en `test_provider.py`). **Arreglo**: `conftest.py` restaura el entorno completo y los globals de litellm tras cada test; `setup_function` pasa a fixture con `monkeypatch`. La suite daba el mismo resultado en orden normal e inverso, así que ninguna fuga enmascaraba un fallo: eran acoples latentes.
- `test_permissions_lists_empty_then_registered_pattern` tenía un helper muerto (`run_with_inputs`, nunca llamado) que parcheaba un `builtins.interaction_input_holder` inexistente. Eliminado.
- `test_blocklist_rm_rf_dentro_de_cwd_permitido` verificaba que el comando no se bloqueara, pero no que se ejecutara. Ahora comprueba que el directorio se borró.

### Revisado y descartado
Los 3 tests de `test_handoff_schema.py` sin `assert` (`jsonschema.validate` lanza si falla), los 5 `try: ...; assert False` de `test_tools.py` (el `except` es angosto, el `assert` se propaga), el test de video (distingue el camino de fallback del de éxito), y los de sandbox (limpian ramas y worktrees). **Pendiente, sin tocar**: `test_extract_ocr_graceful` sigue siendo un smoke test que no puede fallar (todas las salidas incluyen el nombre del archivo) y ejecuta el OCR real de Windows; no se tocó por ser inofensivo.

### Verificación
- **Suite**: 437 passed + 1 xfailed (el bug conocido del troceador). Base 437; el test nuevo sale de dividir el del "Dr." en dos.
- **Mismos indicadores del plugin, antes → después** (el "antes" son 455 tests porque incluye los 18 de la rama V7-12): `git push` real 1 → **0**; red por subproceso (`curl`, `yt-dlp`) 2 → **0**; `rm` fuera de temporales 7 → **0**; tests que escriben fuera de temporales 26 → **0**; tests con fuga de globals de litellm 17 → **0**; tests con fuga de entorno 12 → **0** reales (el plugin marca 2, el primero y el último de la sesión, por `PYTHONPATH`: es el fixture de sesión que lo pone al arrancar y lo restaura al terminar); intentos de red por sockets 0 → 0.
- **Mutaciones** (cada una rompe una garantía en el código de producción, corre el test y restaura con git): regla 2 rota (modelo por defecto silencioso), URLs de YouTube sin enrutar a su extractor, y blocklist que ignora `YUNTA_ALLOW_FORCE=1` — **las tres las detecta el test correspondiente**. La versión anterior de `test_missing_model_fails_clearly` no detectaba la primera.
- **`scratch/`**: con un archivo canario en `scratch/` cuyo código de módulo deja una marca, `python -m pytest` en la raíz (el mismo camino que usa el hook) recolecta 438 tests, no el canario, y la marca no aparece.
